...
# initialize Spark session for data validation
spark = SparkSession.builder.appName("NormalizationValidation").getOrCreate()

# loading all tables from CSV files
DimPatient = spark.read.option("header", "true").csv(f"{bp}/DimPatient.csv")
DimInsurance = spark.read.option("header", "true").csv(f"{bp}/DimInsurance.csv")
DimBilling = spark.read.option("header", "true").csv(f"{bp}/DimBilling.csv")
DimProvider = spark.read.option("header", "true").csv(f"{bp}/DimProvider.csv")
DimLocation = spark.read.option("header", "true").csv(f"{bp}/DimLocation.csv")
DimPrimaryDiagnosis = spark.read.option("header", "true").csv(
    f"{bp}/DimPrimaryDiagnosis.csv"
)
DimSecondaryDiagnosis = spark.read.option("header", "true").csv(
    f"{bp}/DimSecondaryDiagnosis.csv"
)
DimTreatment = spark.read.option("header", "true").csv(f"{bp}/DimTreatment.csv")
DimPrescription = spark.read.option("header", "true").csv(f"{bp}/DimPrescription.csv")
DimLabOrder = spark.read.option("header", "true").csv(f"{bp}/DimLabOrder.csv")
FactVisit = spark.read.option("header", "true").csv(f"{bp}/FactVisit.csv")

# checking for null values in primary keys for ensuring if values are unique
...
# checking for null values in foreign keys in FactVisit
print("\nChecking for null values in FactVisit foreign keys:")
fk_columns = [
    "patient_id",
    "insurance_id",
    "billing_id",
...
print("\n Verifying patient status in DimPatient...")
